...
cat data.csv|sed 's/\([0-9]\)\([0-9]\)\([0-9]\)\([0-9]\);/\3\4;/'|sed 's/\([0-9]\)\/\([0-9]\)\/\([0-9][0-9]\)/0\1\/0\2\/\3/g'|sed 's/;\([a-z]\);/;\U\1;/'|sed 's/;;/;\\N;/g'|sed 's/,/./g'|sed 's/;\\n;/;\\N;/'|sed 's/;n/;\\N/g'|sed 's/;N;/;\\N;/'|sed 's/\([0-9][0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9]\);/\1-\2-\3;/'|sed 's/;[[:space:]]/;\\N/'|sed 's/\([0-9][0-9]\)-\([0-9][0-9]\)-\([0-9][0-9]\);/20\3-\2-\1;/g'|sed 's/;/,/g'
